...
class MainActivity : Activity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1001 && resultCode == RESULT_OK) {
            bean = data?.getSerializableExtra("data") as? Bean
            Toast.makeText(this@MainActivity, bean?.name, Toast.LENGTH_SHORT).show()
        }
        if (requestCode == 1002 && resultCode == RESULT_OK) {
            positionBean = data?.getSerializableExtra("data") as PositionBean
            Toast.makeText(this@MainActivity, positionBean?.name, Toast.LENGTH_SHORT).show()
        }
        if (requestCode == 1003 && resultCode == RESULT_OK) {
            multiBean = data?.getSerializableExtra("data") as? ArrayList<Bean>?
            if (multiBean != null) {
